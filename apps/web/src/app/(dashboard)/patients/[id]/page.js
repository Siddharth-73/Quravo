...
function PatientDetailPage() {
    ...
    const { data: patientsList = [], isLoading: isLoadingPatients } = (0, hooks_1.usePatients)();
    const patient = patientsList.find(p => p.id === patientId);
    const { data: timelineEvents = [], isLoading: isLoadingTimeline } = (0, hooks_1.usePatientTimeline)(patientId);
    const { data: attachments = [], isLoading: isLoadingAttachments } = (0, hooks_1.usePatientAttachments)(patientId);
    const uploadAttachmentMutation = (0, hooks_1.useUploadAttachment)();
    ...
    if (isLoadingPatients) {
        return <div className="p-10 flex justify-center"><lucide_react_1.Loader2 className="w-8 h-8 animate-spin text-primary"/></div>;
    }
    if (!patient) {
    ...
    return (<div className="space-y-6 max-w-5xl">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground pt-1">
                ...
                <span>•</span>
                <span className="flex items-center gap-1"><lucide_react_1.Phone className="w-3 h-3"/> {patient.phone}</span>
                <span>•</span>
                <span className="flex items-center gap-1"><lucide_react_1.Mail className="w-3 h-3"/> {patient.email}</span>
              </div>
            </div>
          </div>

          <div className="flex items-center gap-2 shrink-0">
            <link_1.default href={`/encounters/new?patientId=${patientId}`} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:opacity-90 transition-opacity shadow-sm">
              <lucide_react_1.Stethoscope className="w-3.5 h-3.5"/>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-4 flex items-start gap-3">
          <lucide_react_1.AlertTriangle className="w-4 h-4 text-rose-500 shrink-0 mt-0.5"/>
        ...
        <div className="rounded-xl border border-border bg-card p-4 flex items-start gap-3">
          <lucide_react_1.Activity className="w-4 h-4 text-primary shrink-0 mt-0.5"/>
          <div className="text-xs space-y-0.5">
        ...
        <div className="rounded-xl border border-border bg-card p-4 flex items-start gap-3">
          <lucide_react_1.Pill className="w-4 h-4 text-accent shrink-0 mt-0.5"/>
          <div className="text-xs space-y-0.5">
      ...
      <div className="flex items-center gap-2 border-b border-border pb-3 text-xs">
        {[
            { id: 'timeline', label: 'Medical History Timeline', icon: lucide_react_1.Calendar },
            { id: 'files', label: `Attachments (${attachments.length})`, icon: lucide_react_1.FileText },
        ].map((tab) => {
            const Icon = tab.icon;
            return (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg font-medium transition-colors ${activeTab === tab.id
      ...
      {/* Timeline View */}
      {activeTab === 'timeline' && (<div className="rounded-xl border border-border bg-card p-6 space-y-6 shadow-xs">
          <h3 className="font-semibold text-sm text-foreground">Health Event Timeline</h3>
          {isLoadingTimeline ? (<div className="flex justify-center p-4"><lucide_react_1.Loader2 className="w-5 h-5 animate-spin text-muted-foreground"/></div>) : (<div className="relative pl-6 border-l-2 border-primary/30 space-y-6">
              {timelineEvents.length > 0 ? timelineEvents.map(event => (<div key={event.id} className="relative">
                ...
                </div>)) : (<div className="text-xs text-muted-foreground">No events in timeline.</div>)}
            </div>)}
        </div>)}

      {/* Attachments View */}
      {activeTab === 'files' && (<div className="rounded-xl border border-border bg-card p-6 space-y-6 shadow-xs">
          <div className="flex items-center justify-between">
            ...
            <label className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border bg-card text-xs font-medium hover:bg-muted cursor-pointer">
              {uploadAttachmentMutation.isPending ? <lucide_react_1.Loader2 className="w-3.5 h-3.5 animate-spin"/> : <lucide_react_1.FileUp className="w-3.5 h-3.5"/>}
          ...
          {isLoadingAttachments ? (<div className="flex justify-center p-4"><lucide_react_1.Loader2 className="w-5 h-5 animate-spin text-muted-foreground"/></div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {attachments.length > 0 ? attachments.map(file => (<div key={file.id} className="p-4 rounded-xl border border-border bg-muted/20 flex items-start gap-3">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <a href={file.url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-primary mt-1 inline-flex items-center gap-1 font-semibold hover:underline mt-2">
                       <lucide_react_1.Download className="w-3 h-3"/> Download
                    </a>
                  </div>
                </div>)) : (<div className="text-xs text-muted-foreground col-span-full">No attachments found.</div>)}
            </div>)}
        </div>)}
    </div>);
}
